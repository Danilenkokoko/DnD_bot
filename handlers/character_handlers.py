...
async def calculate_and_show_stats(callback: CallbackQuery, state: FSMContext):
    ...
    text = STATS_RESULT_TEMPLATE.format(
        class_name=class_name,
        background=background,
        STR=stats['STR'], mod_str=mod(stats['STR']),
        DEX=stats['DEX'], mod_dex=mod(stats['DEX']),
        CON=stats['CON'], mod_con=mod(stats['CON']),
        INT=stats['INT'], mod_int=mod(stats['INT']),
        WIS=stats['WIS'], mod_wis=mod(stats['WIS']),
        CHA=stats['CHA'], mod_cha=mod(stats['CHA']),
        hp=stats_result['hp'],
        ac=stats_result['ac']
    )
