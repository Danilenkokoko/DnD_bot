...
from repositories.equipment_repository import FightingStyleRepository, InvocationRepository
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "4️⃣ Выберите БОЕВОЙ СТИЛЬ (для Воина, Паладина, Следопыта, Варвара)\n"
...
async def select_class(callback: CallbackQuery, state: FSMContext):
    ...
    if subclasses and class_name in ["Жрец", "Друид", "Колдун"]:
        ...
        if has_equipment_choice:
            ...
            await ProgressionService.go_to_spells(callback.message, state)
...
# =========================================================
# ЗАКЛИНАНИЯ (ПЕРЕДАЁМ В SPELL_SELECTION_SERVICE)
# =========================================================

# Все обработчики заклинаний зарегистрированы в services/spell_service.py
# Следует подключить их через router.include_router() в bot.py
# Для краткости здесь оставляем комментарий, что они будут подключены отдельно
...
async def select_fighting_style(callback: CallbackQuery, state: FSMContext):
    ...
    await ProgressionService.go_to_invocations(callback.message, state)
    await callback.answer()
...
async def skip_invocations(callback: CallbackQuery, state: FSMContext):
    await ProgressionService.go_to_background(callback.message, state)
    await callback.answer()
